function [u_hat,eta,x,mse_eta] = fast_hstln_mo(u,R,varargin)
...
%       
%
%   Update 02/11/2014 replace the mean with missing values for better x0
%   estimation
...
mu = sum(u,2)/sum(Omega);
u = u + repmat(mu,[1 N_u]).*repmat(1-Omega,[D_u 1]);
...
if ~isempty(x0) 
    ...
    A(1:R,1:R) = A(1:R,1:R) + eye(R)*1e-5;  % to fix rank deficiency
    x = A\b;
end
...
D = diag(reshape(repmat(Omega+1e-5,[D_u 1]),size(eta)));
D = sparse(D);
...
%     try
    ...
    if iter==1
        ...
        M = M + I;
        [MQ,MR]  = qr(M,0);                 % qr can handle rank deficiency
        dparam   = MR\(MQ'*-[w*r;D*eta]);
%         dparam = M\(-[w*r;D*eta]);
%         error(lastwarn);
%     catch err
%         35;
...
u_hat = u + eta;
mse_eta = norm(eta.*repmat(1.*Omega,[D_u 1]),'fro')^2/(sum(Omega)*D_u);
